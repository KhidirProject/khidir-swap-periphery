...
import '@KhidirProject/khidir-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@KhidirProject/khidir-swap-lib/contracts/access/Ownable.sol';

import './KhidirToken.sol';

contract KhidirMaster is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Dirs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accDirPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDirPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. Dirs to distribute per block.
        uint256 lastRewardBlock; // Last block number that Dirs distribution occurs.
        uint256 accDirPerShare; // Accumulated Dirs per share, times 1e12. See below.
        bool exists; //
    }
    // DIR tokens created first block.
    uint256 public dirStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when DIR mining starts.
    uint256 public startBlock;
    // Block number when bonus DIR period ends.
    ...
    // DIR tokens created at bonus end block.
    uint256 public dirBonusEndBlock;
    // max reward block
    ...
    // Accumulated Dirs per share, times 1e12.
    uint256 public accDirPerShareMultiple = 1E12;
    // The DIR TOKEN!
    KhidirToken public dir;
    // Dev address.
    ...
    constructor(
        ...
        // dir created when bonus end first block.
        // (dirStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        dirBonusEndBlock = dirStartBlock
        ...
        // max mint block number, _dirInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_dirInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(dirBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function add(
        ...
        pool.accDirPerShare = 0;
        pool.exists = true;
        poolAddresses.push(_pair);
    }

    // Update the given pool's DIR allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_dirInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            totalReward = tempTo
                ...
                .mul(_dirInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _dirInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempDirInitBlock = _dirInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempDirInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    // View function to see pending Dirs on frontend.
    function pendingDir(address _pair, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfoMap[_pair];
        if (!pool.exists) {
            return 0;
        }
        UserInfo storage userInfo = poolUserInfoMap[_pair][_user];
        uint256 accDirPerShare = pool.accDirPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 dirReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accDirPerShare = accDirPerShare.add(dirReward.mul(accDirPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accDirPerShare).div(accDirPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 dirReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        dir.mintTo(devAddr, dirReward.div(100));
        dir.mintTo(address(this), dirReward);
        pool.accDirPerShare = pool.accDirPerShare.add(dirReward.mul(accDirPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to KhidirMaster for DIR allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accDirPerShare).div(accDirPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
                safeDirTransfer(msg.sender, pending);
            }
        }
        IBEP20(_pair).safeTransferFrom(address(msg.sender), address(this), _amount);
        userInfo.amount = userInfo.amount.add(_amount);
        userInfo.rewardDebt = userInfo.amount.mul(pool.accDirPerShare).div(accDirPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from KhidirMaster.
    ...
    // Safe dir transfer function, just in case if rounding error causes pool to not have enough DIRs.
    function safeDirTransfer(address _to, uint256 _amount) internal {
        uint256 dirBal = dir.balanceOf(address(this));
        if (_amount > dirBal) {
            dir.transfer(_to, dirBal);
        } else {
            dir.transfer(_to, _amount);
        }
    }
